                        ...
                        {timelineSteps.map((step, index) => (
                            <div key={index} className={`flex flex-col lg:flex-row items-center ${step.side === 'right' ? 'lg:flex-row-reverse' : ''}`}>
                                ...
                                <motion.div
                                    ...
                                    className={`w-full lg:w-1/2 px-4 lg:px-12 text-center ${step.side === 'left' ? 'lg:text-right' : 'lg:text-left'}`}
                                >
                                    ...
                                    <h3 className="text-2xl lg:text-3xl font-bold mb-4 font-outfit dark:text-white">{step.title}</h3>
                                    <p className="text-slate-500 dark:text-slate-400 text-base leading-relaxed max-w-md mx-auto lg:mx-0">
                                        {step.desc}
                                    </p>
                                </motion.div>

                                {/* Vertical Center Point */}
                                <div className="relative z-10 flex items-center justify-center my-8 lg:my-0">
                                    <motion.div
                                        initial={{ scale: 0 }}
                                        whileInView={{ scale: 1 }}
                                        viewport={{ once: true }}
                                        className="w-14 h-14 lg:w-20 lg:h-20 bg-blue-600 rounded-3xl rotate-12 flex items-center justify-center shadow-xl shadow-blue-500/30"
                                    >
                                        <div className="-rotate-12 w-6 h-6 lg:w-8 lg:h-8 flex items-center justify-center">
                                            {step.icon}
                                        </div>
                                    </motion.div>
                                </div>
